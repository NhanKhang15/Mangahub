...
func (r *mongoRepo) Create(ctx context.Context, a *domain.Artist) (primitive.ObjectID, error) {
	now := time.Now().UTC()
	a.CreatedAt = now
	a.UpdatedAt = now
...
func (r *mongoRepo) Get(ctx context.Context, id primitive.ObjectID) (*domain.Artist, error) {
	var a domain.Artist
...
func (r *mongoRepo) Update(ctx context.Context, id primitive.ObjectID, set bson.M) (*domain.Artist, error) {
...
func (r *mongoRepo) List(ctx context.Context, q string, page, limit int) ([]*domain.Artist, int64, error) {
	filter := bson.M{}
	if q != "" {
